...
if (time_condition)
    ...
    if (src > trendline_ema)
        strategy.entry("LONG - " + tostring(src) + " Minimum profit = " + tostring(minimumProfitLong), strategy.long, when=longSignal, alert_message="{{strategy.order.action}} au prix de {{strategy.order.price}} effectué sur {{ticker}}.")
...
if strategy.position_size > 0
    ...
    if (src > entryPrice) 
        strategy.close_all(when = src >= minimumProfitLong, comment = "Close Long by Short signal - " + tostring(src), alert_message="{{strategy.order.action}} au prix de {{strategy.order.price}} effectué sur {{ticker}}.")
...
//     // close order only if close_price_candle > price_order_long
//     if (src > entryPrice) 
//         strategy.close_all(when = src <= minimumProfitShort, comment = "Close Short by Short signal - " + tostring(src), alert_message="{{strategy.order.action}} au prix de {{strategy.order.price}} effectué sur {{ticker}}.")
