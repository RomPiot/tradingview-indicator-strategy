//@version=4
strategy(title="Speed & Slow EMA", shorttitle="S&S EMA", precision=2, overlay=false, initial_capital=1000, format=format.price, currency=currency.USD, default_qty_type=strategy.percent_of_equity, default_qty_value=100)
...
o = security(ha, timeframe.period, open)
h = security(ha, timeframe.period, high)
l = security(ha, timeframe.period, low)
c = security(ha, timeframe.period, close)
...
// Calculating
fast_ma = sma_source == "SMA" ? sma(src_ha, fast_length) : ema(src_ha, fast_length)
slow_ma = sma_source == "SMA" ? sma(src_ha, slow_length) : ema(src_ha, slow_length)
macd = fast_ma - slow_ma
signal = sma_signal == "SMA" ? sma(macd, macd_length) : ema(macd, macd_length)
trendline_ema = sma_source == "SMA" ? sma(src_ha, trendline_ema_length) : ema(src_ha, trendline_ema_length)
hist = macd - signal
...
if (time_condition)
    ...
    strategy.entry("LONG - " + tostring(f_round(src)), strategy.long, when=longSignal, alert_message="{{strategy.order.action}} au prix de {{strategy.order.price}} effectué sur {{ticker}}.")
    strategy.entry("SHORT - " + tostring(f_round(src)), strategy.short, when=shortSignal, alert_message="{{strategy.order.action}} au prix de {{strategy.order.price}} effectué sur {{ticker}}.")
     
    // Take Profit and Stop Loss orders
    if strategy.position_size > 0  
        strategy.exit(id = "Close Long - " + tostring(f_round(src)), stop=longStop, limit=longTake, alert_message="{{strategy.order.action}} au prix de {{strategy.order.price}} effectué sur {{ticker}}.")
    ...
    if strategy.position_size < 0
        strategy.exit(id="Close Short - " + tostring(f_round(src)), stop=shortStop, limit=shortTake, alert_message="{{strategy.order.action}} au prix de {{strategy.order.price}} effectué sur {{ticker}}.")
        // strategy.close_all(when = src > entryPrice, comment = "Close Long by Short signal - " + tostring(src), alert_message="{{strategy.order.action}} au prix de {{strategy.order.price}} effectué sur {{ticker}}.")
